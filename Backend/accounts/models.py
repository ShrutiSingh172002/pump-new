...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    user_unique_code = models.CharField(max_length=17, unique=True, editable=False)
    ...
    def save(self, *args, **kwargs):
        if not self.user_unique_code:
            count = CustomUser.objects.exclude(pk=self.pk).count()
            self.user_unique_code = f"{self.gst_number}{count + 1:02d}"
