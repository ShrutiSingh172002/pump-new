...
import { FaArrowLeft, FaYoutube, FaInstagram, FaFacebook, FaShoppingCart } from "react-icons/fa";
...
const PumpSpares = () => {
  ...
  const downloadReceipt = async (materialId) => {
    try {
      ...
      const blob = await pumpSparesService.generateReceipt(materialId);
      
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `Pump_Spares_Receipt_${new Date().toISOString().slice(0, 10)}.docx`;
  ...
  return (
    <>
      <div className="pump-spares-container">
        ...
        <div className="back-arrows" onClick={() => navigate("/products")}>
          <FaArrowLeft />
